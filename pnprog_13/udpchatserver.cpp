...
int operator<(struct sockaddr_in left, struct sockaddr_in right) {
    return (left.sin_addr.s_addr < right.sin_addr.s_addr)
            || (left.sin_addr.s_addr == right.sin_addr.s_addr && left.sin_port
            < right.sin_port);
}
...
struct sendTo { // Funktor um for_each auf clients anzuwenden
    ...
    sendTo(Datagram *dg, UDPSocket & udpref) :
    dgram(dg), udp(udpref) {
    }
...
int main(int argc, char**argv) {
    if (argc < 2) {
        ...
        UDPSocket udp = UDPSocket(atoi(argv[1]));
        if (!udp.PrepareUDPServerSocket())
            return 1;
        do {
            Datagram *dgram = udp.receiveDatagram(60000);
            if (dgram) { // ein Datagram wurde erfolgreich empfangen
                cout << "got:" << dgram->message << " from:"
                        << dgram->getPeerInfo() << endl;
                ...
                if (found == clients.end()) {
                    ...
                    if (dgram->message.length() == 1
                            && dgram->message.find('.') == 0)
                        clients.erase(dgram->peer);
                    else
